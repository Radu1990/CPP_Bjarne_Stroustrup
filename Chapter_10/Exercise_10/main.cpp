...
public:
	...
	Token get(ifstream& ist); // fetches a token from the Token stream
...
Token Token_stream::get(ifstream& ist) {
    ...
    switch (ch) {
        ...
        case '9': {
            ist.unget();
            double val;
            ist >> val; // write the number into a double
...
public:
    ...
    double declare(ifstream& ist); // declare a new value in ST
    double reassignment(ifstream& ist); // reassign value to already declared element in ST
    bool is_declared(string s); // check if value is already declared in ST
    bool declared_is_const(const string& s);
};
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
bool Symbol_table:: declared_is_const(const string& s) {
    for (int i = 0; i < var_table.size(); ++i)
        if (var_table[i].name == s && var_table[i].kind) return true;
    return false;
}
//---------------------------------------------------------------------------
double expression(ifstream& ist);
//---------------------------------------------------------------------------
double Symbol_table:: declare(ifstream& ist) {
    Token t0 = ts.get(ist);
...
double Symbol_table:: reassignment(ifstream& ist) {
...
double primary(ifstream& ist) {
	...
	switch (t.kind) {
	    case raise_to_power : { // raise x to the power i
	        double x = primary(ist);
	        double i = primary(ist);
            narrow_cast<int>(i);
            double ret = pow(x,i);
            return ret;
	    }
...
double statement(ifstream& ist) {
	...
	switch(t.kind) {
	    case reassignment_decl : {
             return st.reassignment(ist); // = x = y; means reassign value y to x
	    }
	    case let: {
		    return st.declare(ist); // # x = y; means declare var x = y;
	    }
        default: {
            ts.unget(t);
            return expression(ist); // just do the math
        }
	}
}
...
void calculate(ifstream& ist, ofstream& ost) {
    while (true) {
        try {
            ...
            // predefine - end
            ost << prompt;
            Token t = ts.get(ist); // feed 1 token
            ...
            // help
            if (t.kind == help) {
                ost << h_msg; // this displays help message
                continue;
            }
            // quit
            if (t.kind == quit) {
            ...
            ost << result << statement(ist) << endl; // as soon as unknown characters are feeded it returns the result
            break;
        }
...
int main() {
    try {
        ...
        string oname = "../textfiles/output.txt";
        ifstream ist{iname};
        ofstream ost{oname};
        ost << w_msg; // display welcome message help
        calculate(ist, ost);
